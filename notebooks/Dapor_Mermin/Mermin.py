...
# %%
EE_500 = grid.EE_prec[:500]
DIIMFP_xchg_0to500 = np.zeros((len(EE_500), len(grid.EE_prec)))

progress_bar = tqdm(total=len(EE_500), position=0)

for i, E in enumerate(EE_500):
    for j, hw in enumerate(grid.EE):

        DIIMFP_xchg_0to500[i, j] = get_PMMA_DIIMFP(E, hw, exchange=True)
