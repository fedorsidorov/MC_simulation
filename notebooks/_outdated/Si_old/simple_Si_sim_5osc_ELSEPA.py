...
# Si_el_IMFP = np.load('notebooks/Si_old/arrays_Si/Si_el_IMFP_nm.npy')
# Si_el_DIMFP_cumulated = np.load('notebooks/Si_old/arrays_Si/Si_el_DIMFP_cumulated.npy')
...
# Si_ee_IMFP = np.load('notebooks/Si_old/arrays_Si/5_osc/Si_ee_IMFP_5osc_nm.npy')
# Si_ee_DIMFP_cumulated = np.load('notebooks/Si_old/arrays_Si/5_osc/Si_ee_DIMFP_cumulated_5osc.npy')

Si_ee_IMFP = np.zeros((arr_size, 5))

for j in range(5):
    Si_ee_IMFP[:, j] = np.load(
        '/Users/fedor/PycharmProjects/MC_simulation/notebooks/Akkerman_Si_5osc/u/u_' + str(j) + '_nm_precised.npy'
    )
Si_ee_DIMFP_cumulated = np.zeros((5, arr_size, arr_size))

for n in range(5):
    Si_ee_DIMFP_cumulated[n, :, :] = np.load(
    ...
    Si_ee_DIMFP_cumulated[np.where(np.abs(Si_ee_DIMFP_cumulated - 1) < 1e-10)] = 1

    for i in range(arr_size):
        for j in range(arr_size - 1):

            if Si_ee_DIMFP_cumulated[n, i, j] == 0 and Si_ee_DIMFP_cumulated[n, i, j + 1] == 0:
                Si_ee_DIMFP_cumulated[n, i, j] = -2

            if Si_ee_DIMFP_cumulated[n, i, arr_size - j - 1] == 1 and \
                    Si_ee_DIMFP_cumulated[n, i, arr_size - j - 2] == 1:
                Si_ee_DIMFP_cumulated[n, i, arr_size - j - 1] = -2

    zero_inds = np.where(Si_ee_DIMFP_cumulated[n, -1, :] == 0)[0]
    ...
    if len(zero_inds) > 0:
        ...
        if grid.EE[zero_ind] < Si_ee_E_bind[n]:
            Si_ee_DIMFP_cumulated[n, :, zero_ind] = -2

Si_ee_DIMFP_cumulated[0, :4, 5] = -2

Si_ee_DIMFP_cumulated[1, :301, :297] = -2
Si_ee_DIMFP_cumulated[1, 300, 296] = 0

Si_ee_DIMFP_cumulated[2, :461, :457] = -2
Si_ee_DIMFP_cumulated[2, 460, 456] = 0

Si_ee_DIMFP_cumulated[3, :500, :496] = -2
Si_ee_DIMFP_cumulated[3, 499, 495] = 0

Si_ee_DIMFP_cumulated[4, :745, :742] = -2
Si_ee_DIMFP_cumulated[4, 744, 741] = 0
...
for E0 in [1000]:
    ...
    for n in range(n_files):
        ...
        np.save('data/4Akkerman/1keV_pl_old/e_DATA_' + str(n) + '.npy', e_DATA)
