...
event_array_total = np.zeros(len(mm.z_centers_5nm))
...
for file_cnt in range(n_files):
    ...
    event_array_total += np.histogram(
        a=now_e_DATA[:, ind.e_DATA_z_ind],
        bins=mm.z_bins_5nm
    )[0]
...
plt.plot(mm.z_centers_5nm, event_array_total, 'o-')
plt.show()

# %%
# pf.plot_e_DATA(now_e_DATA, mm.d_PMMA, xx, zz_vac)
