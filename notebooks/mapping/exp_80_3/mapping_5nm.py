...
# %% load arrays
scission_matrix = np.load('data/e_matrix/exp_80_3_5nm/scission_matrix_snaked.npy')
E_dep_matrix = np.load('data/e_matrix/exp_80_3_5nm/E_dep_matrix_snaked.npy')

resist_matrix = np.load('data/exp_80_3/resist_matrix_5nm_80_3.npy')
...
for n in range(n_chains):
    chain_tables.append(
        np.load('data/exp_80_3/chain_tables_5nm/chain_table_' + str(n) + '.npy'))
    progress_bar.update()
...
# %%
np.save('data/exp_80_3/resist_matrix_5nm_80_3_exposed.npy', resist_matrix)

progress_bar = tqdm(total=len(chain_tables), position=0)

for n, chain_table in enumerate(chain_tables):
    np.save('data/exp_80_3/chain_tables_exposed_5nm/chain_table_' + str(n) + '.npy', chain_table)
    progress_bar.update()
