...
# %%
# for weight in ['0.150', '0.200', '0.250', '0.300', '0.350', '0.400']:
for weight in ['0.150', '0.200']:

    print(weight)

    # load arrays_Si
    scission_matrix = np.load('data/choi_weight/e_matrix_val_ion_sci_' + weight + '_2nm.npy')

    resist_matrix = np.load('data/G_calibration/resist_matrix_2nm.npy')
    n_chains = 6081
    ...
    for n in range(n_chains):
        now_chain_table = np.load('data/G_calibration/chain_tables_2nm/chain_table_' + str(n) + '.npy')
    ...
    np.save('data/choi_weight/harris_lens_final_' + weight + '_2nm.npy', lens_final)
