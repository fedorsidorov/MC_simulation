...
for n_shell in [1, 2, 3, 4, 5]:
    ...
    DIIMFP = np.load('/Users/fedor/PycharmProjects/MC_simulation/notebooks/Akkerman_Si_5osc/u_diff/DIIMFP_' +
                     str(n_shell) + '.npy')
    ...
    # np.save('notebooks/Akkerman_Si_5osc/u_diff_cumulated/DIIMFP_' + str(n_shell - 1) + '_cumulated.npy', u_diff_cumulated)

# %%
plt.figure(dpi=300)
# plt.loglog(grid.EE, u_diff[100, :])
plt.semilogx(grid.EE, DIIMFP_cumulated[500, :])
plt.show()

# %%
arr = np.load('notebooks/Akkerman_Si_5osc/u_diff_cumulated/u_diff_4_cumulated.npy')
