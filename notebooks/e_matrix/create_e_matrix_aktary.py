...
# %% get e-matrix
e_matrix_val_exc_sci = np.zeros(ma.hist_2nm_shape)
e_matrix_val_ion_sci = np.zeros(ma.hist_2nm_shape)
e_matrix_E_dep = np.zeros(ma.hist_2nm_shape)

dose_C_cm2 = 100
x_position = 0
r_beam = 2

n_electrons_required = emf.get_n_electrons_2D(dose_C_cm2, 2 * r_beam, ma.l_y)
n_electrons = 0
...
while n_electrons < n_electrons_required:
    ...
    for primary_e_id in range(primary_electrons_in_file):
        ...
        # emf.get_gaussian_xy_shift_to_DATA(now_prim_e_DATA, x_position, r_beam, [ma.y_min, ma.y_max])
        ...
        af.snake_array(
            ...
            xyz_min=[ma.x_min, ma.y_min, -np.inf],
            xyz_max=[ma.x_max, ma.y_max, np.inf]
        )
...
# %%
np.save('data/e_matrix/Aktary/C-C2:4_C-C\':2_C-C3:1/e_matrix_val_exc_sci.npy', e_matrix_val_exc_sci)
np.save('data/e_matrix/Aktary/C-C2:4_C-C\':2_C-C3:1/e_matrix_val_ion_sci.npy', e_matrix_val_ion_sci)
np.save('data/e_matrix/Aktary/C-C2:4_C-C\':2_C-C3:1/e_matrix_dE.npy', e_matrix_E_dep)
