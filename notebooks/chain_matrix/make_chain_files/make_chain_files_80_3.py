...
hist_2nm = np.load('/Volumes/ELEMENTS/chains_950K/rot_sh_sn_chains/exp_80_3/hist_2nm.npy')
n_mon_cell_max = int(np.max(hist_2nm))

plt.imshow(np.average(hist_2nm, axis=0).transpose())
plt.show()
...
for chain_num in range(len(chain_lens)):
    ...
    now_chain = np.load('/Volumes/ELEMENTS/chains_950K/rot_sh_sn_chains/exp_80_3/rot_sh_sn_chain_' +
                        str(chain_num) + '.npy')
...
# np.save('/Volumes/ELEMENTS/chains_950K/resist_matrix/exp_80nm_Camscan/resist_matrix_2nm.npy', resist_matrix)
np.save('resist_matrix_2nm_80_3.npy', resist_matrix)

# %%
progress_bar = tqdm(total=len(chain_tables), position=0)

for n, chain_table in enumerate(chain_tables):
    np.save('data/exp_80_3/chain_tables/chain_table_' + str(n) + '.npy', chain_table)
    progress_bar.update()
