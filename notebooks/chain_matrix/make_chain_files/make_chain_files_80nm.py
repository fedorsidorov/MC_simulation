...
hist_5nm = np.load('/Volumes/ELEMENTS/chains_950K/rot_sh_sn_chains/exp_80nm/hist_5nm.npy')
...
for chain_num in range(len(chain_lens)):
    ...
    now_chain = np.load('/Volumes/ELEMENTS/chains_950K/rot_sh_sn_chains/exp_80nm/rot_sh_sn_chain_' +
                        str(chain_num) + '.npy')
...
np.save('/Volumes/ELEMENTS/chains_950K/resist_matrix/exp_80nm/resist_matrix_5nm.npy', resist_matrix)

# %%
progress_bar = tqdm(total=len(chain_tables), position=0)

for n, chain_table in enumerate(chain_tables):
    np.save('/Volumes/ELEMENTS/chains_950K/chain_tables/exp_80nm/chain_table_' + str(n) + '.npy', chain_table)
    progress_bar.update()
