...
hist_2nm = np.load('/Volumes/ELEMENTS/chains_950K/rot_sh_sn_chains/exp_80nm_Camscan/hist_2nm.npy')
...
for chain_num in range(len(chain_lens)):
    ...
    now_chain = np.load('/Volumes/ELEMENTS/chains_950K/rot_sh_sn_chains/exp_80nm_Camscan/rot_sh_sn_chain_' +
                        str(chain_num) + '.npy')
...
for n, chain_table in enumerate(chain_tables):
    np.save('/Volumes/ELEMENTS/chains_950K/chain_tables/exp_80nm_Camscan/chain_table_' + str(n) + '.npy', chain_table)
    progress_bar.update()
