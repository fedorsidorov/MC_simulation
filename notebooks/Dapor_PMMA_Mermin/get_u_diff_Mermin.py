...
# %% calculate u_diff_prec
def get_PMMA_DIIMFP(E_eV, hw_eV, exchange):
    if hw_eV > E_eV:
        return 0
...
for i, E_D in enumerate(EE_D):
    for j, E in enumerate(EE):
        ...
        DIIMFP[i, j] = get_PMMA_DIIMFP(E_D, E, exchange=False)
...
# %%
EE_750to1000 = grid.EE_prec[750:1000]
DIIMFP_xchg_750to1000 = np.zeros((len(EE_750to1000), len(grid.EE_prec)))

progress_bar = tqdm(total=len(EE_750to1000), position=0)

for i, E in enumerate(EE_750to1000):

    print(E)

    for j, hw in enumerate(grid.EE):

        DIIMFP_xchg_750to1000[i, j] = get_PMMA_DIIMFP(E, hw, exchange=True)

    progress_bar.update()

# %%
np.save('notebooks/Dapor_PMMA_Mermin/u_diff_prec/PMMA_DIIMFP_xchg_750to1000.npy', DIIMFP_xchg_750to1000)
