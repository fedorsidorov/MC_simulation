...
yy_pre_pre = np.array((0, l0/8, l0/4 - 0.01, l0/4, l0/4 + l0/8, l0/2, l0/2 + l0/8, l0*3/4, l0*3/4 + 0.01,
                       l0*3/4 + l0/8, l0 - 0.01))

yy_list = []

for n in range(-int(n_periods_y / 2), int(n_periods_y / 2)):
    yy_list.append(yy_pre_pre + l0*n)

yy_pre = np.concatenate(yy_list)
yy_pre = np.concatenate((yy_pre, [l0 * n_periods_y/2]))

zz_pre_pre_pre = [z_min, z_min, z_min, z_max, z_max, z_max, z_max, z_max, z_min, z_min, z_min]
zz_pre_pre = zz_pre_pre_pre * n_periods_y
zz_pre_pre.append(z_min)
zz_pre = np.array(zz_pre_pre) * 1e-3
...
for i in range(len(xx)):
    # zz[i, :] = mf.lin_lin_interp(yy_pre, zz_pre)(yy)
    zz[i, :] = zz_pre
...
file += 'PARAMETER now_vmob = ' + str(0.00044) + '\n'
...
with open('notebooks/SE/set_SE_constraints_DEBER.txt', 'r') as myfile:
    file += myfile.read()

# % write to file
# with open('notebooks/SE/SE_input_3D_REF_DEBER_new.fe', 'w') as myfile:
#     myfile.write(file)
