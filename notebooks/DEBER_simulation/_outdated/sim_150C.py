...
def save_mobilities():
    ...
    plt.plot(xx_centers, mobs_array_filt, label='Mn mobility filt')
    plt.plot(xx_centers, mobs_centers, '--', label='FINAL')
...
def save_scissions():
    plt.figure(dpi=300)
    # plt.imshow((now_scission_matrix - now_scission_matrix[::-1, :]).transpose())
    plt.plot(xx_centers, np.sum(now_scission_matrix, axis=1))

    if not os.path.exists(path + 'scissions/'):
        os.makedirs(path + 'scissions/')

    plt.savefig(path + '/scissions/scission_array_' + str(now_time) + '_s.jpg', dpi=300)
    plt.close('all')


def save_profiles():
    plt.figure(dpi=300)
    plt.plot(new_xx_total, new_zz_total, '.-', color='C0', ms=2, label='SE profile')
    plt.plot(xx_centers, new_zz_inner_final, '.-', color='C4', ms=2, label='inner interp')
    plt.plot(xx_bins, new_zz_surface_final, '.-', color='C3', ms=2, label='PMMA interp')
    ...
    plt.plot(-now_x0_array, d_PMMA - now_z0_array, 'm.')
    plt.plot(xx_bins, np.zeros(len(xx_bins)), 'r')
...
def save_profiles_final(new_xx_total, new_zz_total_final, new_zz_surface_final, new_zz_inner_final, time):
    ...
    plt.figure(dpi=300)
    plt.plot(new_xx_total, new_zz_total_final, '.-', color='C0', ms=2, label='SE profile')
    plt.plot(xx_centers, new_zz_inner_final, '.-', color='C4', ms=2, label='inner interp')
    plt.plot(xx_bins, new_zz_surface_final, 'r.-', color='C3', ms=2, label='PMMA interp')
...
def make_SE_1s_iteration(zz_vac_bins, zz_inner_centers, mobs_centers, fname):
    ...
    # if now_time in [99, 199]:
    #     finalize(zz_vac_bins, zz_inner_centers, now_time + 1)

    zz_vac_bins_final = d_PMMA - new_zz_surface_final
    zz_inner_centers_final = d_PMMA - new_zz_inner_final

    return zz_vac_bins_final, zz_inner_centers_final


# %% constants
xx_366_100 = np.load('notebooks/DEBER_simulation/exp_profiles/xx_366.npy')
zz_366_100 = np.load('notebooks/DEBER_simulation/exp_profiles/zz_366.npy')
...
eta_Mn_matrix = np.zeros((len(xx_centers), len(zz_centers)))
mob_Mn_matrix = np.zeros((len(xx_centers), len(zz_centers)))

Mn_final_array = np.zeros(len(xx_centers))
...
# while now_time < 200:
    ...
    for i in range(len(xx_centers)):
        zz_vac_center_inds[i] = np.where(zz_centers > zz_vac_centers[i])[0][0]
    ...
    for i in range(len(xx_centers)):
        now_scission_matrix[i, :zz_vac_center_inds[i]] = 0

    now_scission_matrix += now_scission_matrix[::-1, :]
    # save_scissions()
    # TODO get scission_matrix END

    # TODO get Mn matrix START
    surface_inds = np.zeros(len(xx_centers), dtype=int)

    for i in range(len(xx_centers)):
        for j in range(len(zz_centers)):

            if zz_vac_centers[i] > zz_centers[j] and j < len(zz_centers) - 1:
                surface_inds[i] = j + 1
    ...
    save_surface_inds()
    # save_Mn_matrix()
    # TODO get Mn matrix END

    # TODO get monomer array START
    ...
    # save_monomers()
    # TODO get monomer array END

    # TODO get raw new profile START
    ...
    # TODO Surface Evolver part START
    # xx_SE = np.zeros(len(xx_bins) + len(xx_centers) - 1)
    # zz_vac_SE = np.zeros(len(xx_bins) + len(xx_centers) - 1)
    # mobs_SE = np.zeros(len(xx_bins) + len(xx_centers) - 1)
    #
    ...
    #     xx_SE[2 * i + 1] = xx_centers[i]
    #     zz_vac_SE[2 * i] = zz_vac_bins[i]
    #     zz_vac_SE[2 * i + 1] = new_zz_inner_centers[i]
    #
    # mobs_SE[0] = mobs_centers[0]
    # mobs_SE[-1] = mobs_centers[-1]
    # mobs_SE[1:-1] = mcf.lin_lin_interp(xx_centers, mobs_centers)(xx_SE[1:-1])
    ...
    #
    # zz_vac_bins = d_PMMA - new_zz_surface_final
    # zz_inner_centers = d_PMMA - new_zz_inner_final
