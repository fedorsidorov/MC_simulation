...
total_time = dose_l / j_exp_l  # 1024 s
Q = dose_s * mm.area_cm2
n_electrons = Q / constants.e_SI  # 54 301
...
for n_step in range(103):
    ...
    plt.xlabel('z, nm')
    plt.legend()
    plt.savefig('notebooks/DEBER_simulation/5_3000_1e-1/' + str(n_step) + '.jpg', dpi=300)

    zz_vac = mm.d_PMMA - zz_vac_final
    zz_vac_list.append(zz_vac)

# %%
# exp = np.loadtxt('exp_900nm_5um_0.05uC_cm2.txt')
# sim = np.loadtxt('sim_900nm_5um_0.05uC_cm2.txt')
#
# plt.figure(dpi=300)
# plt.plot(exp[:, 0] - 2.9, exp[:, 1])
# plt.plot(sim[:, 0], sim[:, 1] - 0.67)
# plt.show()
