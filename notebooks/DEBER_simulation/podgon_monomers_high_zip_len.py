...
n_steps = 235
# zip_length = 4000
SE_mobility_mat = np.ones((len(mm.x_centers_5nm), n_steps)) * 0.01
zip_length_mat = np.ones((len(mm.x_centers_5nm), n_steps)) * 1000

# SE_mobility_arr[0] = 4
# SE_mobility_arr[1] = 3
# SE_mobility_arr[2] = 2
# SE_mobility_arr[3] = 1.5
# SE_mobility_arr[4:7] = 1.2
# SE_mobility_arr[7:10] = 0.5

SE_mobility_mat[:, 0:10] = 1
zip_length_mat[:, 0:10] = 500
SE_mobility_mat[:, 10:20] = 1
zip_length_mat[:, 10:20] = 7000
SE_mobility_mat[:, 20:30] = 1
zip_length_mat[:, 20:30] = 10000
SE_mobility_mat[:, 30:40] = 1
zip_length_mat[:, 30:40] = 17000
SE_mobility_mat[:, 40:50] = 1
zip_length_mat[:, 40:50] = 23000

# SE_mobility_arr[50:60] = 0.5
# zip_length_arr[50:60] = 20000
SE_mobility_mat[:, 50:60] = 1
zip_length_mat[:, 50:60] = 35000

coeff = 100
coeff_arr = (np.cos(xx_vac * 2 * np.pi / mm.lx) + 1) * coeff

plt.figure(dpi=300)
plt.plot(xx_vac, coeff_arr)
plt.show()

for i in range(i):
    SE_mobility_mat[i, :] = SE_mobility_mat[i, :] * coeff_arr
...
if now_n_step == 0:
    ...
    zz_vac = np.load('notebooks/DEBER_simulation/podgon_monomer/' + str(now_n_step - 1) + '_zz_vac_' +
                     str(zip_length_mat[now_n_step - 1]) + '_' + str(SE_mobility_mat[now_n_step - 1]) + '.npy')
...
for n_step in range(now_n_step, now_n_step + 10):
    ...
    plt.savefig('notebooks/DEBER_simulation/podgon_monomer/' + str(n_step) +
                '_zz_vac_' + str(zip_length) + '_' + str(SE_mobility_mat[n_step]) + '.jpg')
    ...
    np.save('notebooks/DEBER_simulation/podgon_monomer/' + str(n_step) +
            '_zz_vac_' + str(zip_length) + '_' + str(SE_mobility_mat[n_step]) + '.npy', zz_vac)
