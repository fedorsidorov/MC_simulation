...
for j in range(n_steps):
    coeff_arr = (np.cos(xx_vac * 2 * np.pi / mm.lx) + 1) ** (cos_power * j / n_steps)
    SE_mobility_mat[:, j] = SE_mobility_mat[:, j] * coeff_arr
    SE_mobility_mat[:, j] /= np.max(SE_mobility_mat[:, j])
    # SE_mobility_mat[:, j] *= SE_mob_factor / (j + 1)
...
for i in range(0, n_steps, 20):
    plt.plot(xx_vac, SE_mobility_mat[:, i], label=str(i) + ' s')
...
for n_step in range(n_steps):
    ...
    plt.savefig('notebooks/DEBER_simulation/podgon_monomer/vary_mob/' + str(n_step) + '_zz_vac.jpg')

    plt.close('all')

    zz_vac = mm.d_PMMA - zz_after_evolver

    np.save('notebooks/DEBER_simulation/podgon_monomer/vary_mob/' + str(n_step) + '_zz_vac.npy', zz_vac)
