...
scission_matrix_4D = np.zeros([32, *np.shape(scission_matrix)])
...
for i in range(32):
    ...
    now_scission_matrix = np.zeros(shape)
    ...
    scission_matrix_4D[i, :, :, :] = now_scission_matrix

# %%
ans = np.load('notebooks/DEBER_simulation/scission_matrix_4d.npy')
bns = np.sum(np.average(ans, axis=2), axis=2)
...
zz_vac_matrix = np.load('notebooks/DEBER_simulation/zz_vac_matrix.npy')
zz_vac = zz_vac_matrix[-1, :]

scission_array = np.load('notebooks/DEBER_simulation/toal_scission_array.npy')
...
zz_evolver = mm.d_PMMA - zz_vac_matrix[-1, :]

xx_final_tochno = np.concatenate(([mm.x_min], mm.x_centers_5nm, [mm.x_max], [mm.x_max]))
zz_final_tochno = np.concatenate(([zz_evolver[0]], zz_evolver, [zz_evolver[-1]]))
mobs_array_final_tochno = np.concatenate(([mobs_array[0]], mobs_array_5nm, [mobs_array[-1]], [mobs_array[-1]]))
