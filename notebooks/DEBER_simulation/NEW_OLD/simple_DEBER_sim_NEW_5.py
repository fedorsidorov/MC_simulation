...
def save_mobilities():
    ...
    plt.plot(xx_centers, mobs_final, '--', label='FINAL')
...
def save_profiles():
    ...
    plt.plot(xx_bins, new_zz_surface_final, '.-', ms=2, label='PMMA interp')
...
def get_pos_enter(now_x0):
    ...
    for ne in [50, 100, 150, 200, 250, 300, 350, 400, 450]:
        if ne - 50 <= now_z0 < ne:
            pos_enter = ne - 50
    ...
    # print(now_x0, now_x0_bin, now_z0, pos_enter)
...
path = 'notebooks/DEBER_simulation/sigma_' + str(beam_sigma) + 'nm/bins_' + str(x_step) + '_' +\
       str(z_step) + '_nm/zip' + str(zip_length) + '_k' + str(kernel_size) + '_ph' + \
       str(power_high) + '_pl' + str(power_low) + '_' + str(int(Mn_edge / 1000)) + 'k/'

final_fname = 'notebooks/DEBER_simulation/sigma_' + str(beam_sigma) + 'nm/bins_' + str(x_step) + '_' +\
       str(z_step) + '_nm/zip' + str(zip_length) + '_k' + str(kernel_size) + '_ph' + \
...
while now_time < exposure_time:
    ...
    now_x0_array = np.zeros(60)
    now_z0_array = np.zeros(60)
