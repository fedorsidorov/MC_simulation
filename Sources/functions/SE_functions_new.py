...
# %%
def create_datafile_no_mob_fit(yy_nm, zz_nm, mobs):  # xx and zz in um !!!
    ...
    n_vertices = nx * ny * 2
    VV = np.zeros((n_vertices, 1 + 3 + 1))  # + mobility!
    ...
    file += 'PARAMETER lx = ' + str(np.max(xx) - np.min(xx)) + '\n'
    file += 'PARAMETER ly = ' + str(np.max(yy) - np.min(yy)) + '\n'
    ...
    # % write to file
    with open('notebooks/SE/SE_input_3D_DEBER_new.fe', 'w') as myfile:
        myfile.write(file)
