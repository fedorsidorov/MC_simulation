...
def create_datafile_non_period(yy_pre_pre, zz_pre_pre, mobs_pre):  # xx and zz in um !!!
    ...
    def gauss(x, a, b, c):
        return a + b / c * np.exp(-x ** 2 / c ** 2)

    # popt = curve_fit(exp_gauss, yy_pre, mobs)[0]
    popt = curve_fit(gauss, yy_pre_pre, mobs_pre, p0=[-30, 378, 150])[0]
    a, b, c = popt

    plt.figure(dpi=300)
    plt.plot(yy_pre_pre, mobs_pre, 'o-')
    plt.plot(yy_pre_pre, gauss(yy_pre_pre, *popt))
    plt.plot(yy_pre, gauss(yy_pre, 0.3, 658, 200))
    plt.show()
