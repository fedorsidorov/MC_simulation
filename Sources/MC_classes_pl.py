...
class Structure:
    ...
    def get_ee_scat_phi_theta_hw_phi2_theta2(self, electron, subshell_ind):
        ...
        hw = grid.EE[hw_ind]
        E_bind = self.E_bind[electron.layer_ind][subshell_ind]
        delta_E = hw - E_bind

        phi_2nd = phi - np.pi
        theta = np.arcsin(np.sqrt(delta_E / electron.get_E()))
        theta_2nd = np.arcsin(np.sqrt(1 - delta_E / electron.get_E()))
